...
<div class="rounded-lg border border-[rgba(19,20,23,0.15)] bg-white">
	<div class="flex items-center justify-between border-b border-[rgba(19,20,23,0.08)] px-5 py-4">
		<h3 class="text-base font-semibold text-gray-900">Entitlements</h3>
		{#if canMutate}
			<button
			...
			>
				{showGrantForm ? 'Cancel' : 'Grant Override'}
			</button>
		{/if}
	</div>
	...
	{#if entitlements.length === 0}
		<div class="px-5 py-8 text-center text-sm text-gray-400">No entitlements found</div>
	{:else}
