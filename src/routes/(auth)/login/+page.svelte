...
<svelte:head>
	<title>Sign in - outra.admin</title>
</svelte:head>

<div class="min-h-screen bg-[#F6F5F1] flex items-center justify-center">
	<div class="w-full max-w-md">
		<div class="bg-white rounded-2xl border border-[rgba(19,20,23,0.15)] p-8">
			<!-- Logo -->
			<div class="text-center mb-8">
				<div class="flex items-center justify-center gap-1.5 mb-3">
					<svg xmlns="http://www.w3.org/2000/svg" width="91" height="26" viewBox="0 0 91 26" fill="none" class="text-[#131417]">
						...
						<defs><clipPath id="clip0_login"><rect width="90.2909" height="26" fill="white"/></clipPath></defs>
					</svg>
					<span class="text-[10px] font-bold text-[#4D61F4] uppercase tracking-[0.15em] self-end mb-[3px]">admin</span>
				</div>
				<p class="text-sm text-[#656767]">Sign in to the admin console</p>
			</div>
			...
			>
				<div class="space-y-4">
					<div>
						<label for="email" class="block text-sm font-medium text-[#131417] mb-1">Email</label>
						<input
							id="email"
							name="email"
							type="email"
							...
							disabled={loading}
							autocomplete="email"
							class="w-full px-3 py-2.5 border border-[rgba(19,20,23,0.15)] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#4D61F4] focus:border-transparent disabled:opacity-50 disabled:bg-gray-50"
						/>
					</div>

					<div>
						<label for="password" class="block text-sm font-medium text-[#131417] mb-1">Password</label>
						<input
							id="password"
							name="password"
							type="password"
					...
					<button
						type="submit"
						disabled={loading}
						class="w-full py-2.5 bg-[#2E4BE9] text-white rounded-lg font-medium text-sm hover:bg-[#2540CC] transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
					>
						{#if loading}
